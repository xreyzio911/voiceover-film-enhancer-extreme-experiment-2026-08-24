...
class IncrementalSha256 {
  ...
  update(data: Uint8Array) {
    ...
    if (this.bufferLength > 0) {
      const needed = Math.min(64 - this.bufferLength, data.byteLength);
  ...
  digestHex() {
    ...
    view.setUint32(finalLength - 8, bitLengthHigh, false);
    view.setUint32(finalLength - 4, bitLengthLow, false);
    for (let offset = 0; offset < finalBlock.byteLength; offset += 64) {
      this.transform(finalBlock, offset);
    }
    return [...this.state]
...
export const sha256Blob = async (source: Blob) => {
  ...
  try {
    while (true) {
      ...
      if (done) break;
      if (!(value instanceof Uint8Array)) throw new Error("sha256-invalid-stream-chunk");
      hasher.update(value);
    }
  } finally {
    reader.releaseLock();
