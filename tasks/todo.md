...
- [x] Push and deploy through the exact X Chrome profile, then verify the production URL.
...
- Gemini 3.6 Flash high-thinking audition was session-only and advisory. In a counterbalanced blind A/B, it chose the current Seth and Ethan renders regardless of A/B position; a separate detailed v5 excerpt review also preferred the safer cinematic-body direction. These are supporting taste/DSP judgments, not proof. All eight temporary Files API uploads were deleted, inline reviews created no remote files, the local upload helper was removed, and no key or SDK/runtime integration was added.
- Exact browser v5 corpus evidence measured 31/31 delivered WAVs with 0 errors after the final conservative damping patch. Median old -> v5: body-up spike P95 `3.6600 -> 3.7333 dB`, body-down spike P95 `3.4352 -> 3.5594 dB`, body spread delta `-2.3937 -> -2.5168 dB`, intra-run body median `-1.2801 -> -1.5065 dB`, expressive retention P10 `0.8095 -> 0.8092`, zero lag, and zero median duration delta.
- A fresh measurement consumed the exact browser `selected-manifest.json` directly and reproduced all 31 source hashes, result hashes, per-pair metrics, and corpus-summary values with zero discrepancies. The manifest reader now accepts both the canonical `{ pairs, result }` form and the renderer's `{ selected, output }` form, covered by a RED-to-GREEN regression.
- The v5 pass specifically reduced the earlier v4 body-up outliers: Italian Simone `+1.3643 -> +0.6587 dB` versus old and German Gabriel `+1.2359 -> +0.7431 dB` versus old. Martina's body-down outlier remains elevated but slightly lower than v4 (`+2.2280 -> +2.1765 dB` versus old), so it stays documented as a real follow-up target rather than hidden by the median improvement.
- Final local verification passed after the last parser/DSP patch: voice-stability `29/29`, full audio-QC `484` pass with `1` intentional optional skip, TypeScript, lint with `0` errors and only two pre-existing warnings in the user-owned `scripts/diagGainVsLevel.mts`, production build, and `npm audit --omit=dev --audit-level=high` with 0 vulnerabilities. The scoped diff check was clean; the build retained only the existing Audio Splitter NFT trace warning.
